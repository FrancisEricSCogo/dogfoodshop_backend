...
class PasswordResetOTPMail extends Mailable
{
    ...
    {
        return new Envelope(
            subject: 'Password Reset OTP - Dog Food Shop',
        );
    }
    ...
    public function content(): Content
    {
        ...
        <body>
            <div class='container'>
                <div class='header'>
                    <h2>🐕 Dog Food Shop</h2>
                    <div style='font-size:12px; opacity:0.9;'>Reset Password OTP</div>
                </div>
                <div class='content'>
                    <h3>Hello {$this->userName},</h3>
                    <p>You have requested to reset your password for your Dog Food Shop account.</p>
                    <p>Use this OTP to reset your password. If you didn't request this, you can ignore this email.</p>
                    <div class='otp-box'>
                        <div class='otp-code'>{$this->otp}</div>
                    </div>
                    <p>This OTP will expire in 10 minutes.</p>";

        // Add direct reset link if we know the email
        if (!empty($this->email)) {
            $resetUrl = 'https://francisericscogo.github.io/dogfoodshop/frontend/views/guest/reset_password.html'
            ...
            $html .= "
                    <div style='margin: 20px 0; padding: 16px; background:#f8fafc; border-radius:10px; border:1px solid #e2e8f0;'>
                        <p style='margin:0 0 10px 0; font-size:14px; color:#475569;'>If you closed the reset page, you can go back using this button:</p>
                        <p style='text-align:center; margin:0;'>
                            <a href='{$resetUrl}' style='display:inline-block; padding:10px 20px; background:{$brandGradient}; color:#ffffff; text-decoration:none; border-radius:9999px; font-weight:600; font-size:14px;'>Continue Reset Password</a>
                        </p>
                    </div>";
        }
